...
* Runs the `./gradlew clean test asciidoctor checkstyleMain checkstyleTest -i` command (see <<UsingGradle#, UsingGradle>> for more details on what this command means).
...
+
.  If the build is successful, you should be able to check the coverage details of the tests at http://codecov.io/[Codecov]
