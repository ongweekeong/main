...
Step 1: User enters password for Headquarter Personnel.

Step 2: decipherText() in MainWindow class identifies command as a password input, through the use of .
...
The buffered reader reads the passwordStorage.txt file for the specific password to update. This is stage 1
Once the user has entered a new password , the user is prompted to enter the same password again. This is stage 2.
...
Step 2: decipherText() in MainWindow class identifies command as "logout".
